...
1. **Zend Engine** - ядро PHP (lexer → parser → AST → opcodes → executor)
2. **OpCode** - промежуточный bytecode, не машинный код
3. **OpCache** - кэширование opcode в shared memory, без рекомпиляции
4. **JIT (PHP 8.0+)** - компиляция opcodes в native machine code. Помогает для CPU-intensive, не I/O
5. **Reference Counting** - управление памятью, refcount = 0 → освобождение
6. **Garbage Collector** - решает циклические ссылки (mark-and-sweep)
7. **Copy-on-Write** - $b = $a не копирует, до модификации только ссылка
8. **Packed Array** - последовательные ключи с 0 (быстрее, меньше памяти)
9. **Hash Array** - ассоциативные ключи (медленнее, hash table)
10. **Interned Strings** - одинаковые строки хранятся один раз (экономия памяти)
11. **Autoloading** - spl_autoload_register, PSR-4, composer autoload 
12. **Memory Limit** - memory_limit, ini_set(), мониторинг через memory_get_usage()

**Главное:** OpCache включай всегда, JIT для CPU-bound задач, понимай reference counting для избежания memory leaks.
